...
class PractitionerController {
  ...
  async getPractitionerById(req, res) {
    ...
    try {
      const result = await PractitionerService.getPractitionerById(
        req.params.id
      );
  ...
  async deletePractitioner(req, res) {
    try {
      ...
      if (result == null) res.status(404).send('Practitioner não encontrado');
      else res.json('Practitioner deletado');
    } catch (e) {
      res.status(500).json(e);
    }
  }
  ...
  async updatePractitioner(req, res) {
    ...
    try {
      ...
      if (result == null) res.status(404).send('Practitioner não encontrado');
      else res.json('Practitioner atualizado');
    } catch (e) {
      res.status(500).json(e);
    }
  }
  ...
  async getPractitionerPatients(req, res) {
    try {
      ...
      if (!patients || patients.length === 0) {
        res
          .status(404)
          .send('Nenhum Patient encontrado para este Practitioner.');
      } else {
