...
const BundleController = {
    async processBundle(req, res) {
        ...
        if (!['collection', 'batch'].includes(type)) {
            return res.status(400).json({ message: 'Only collection and batch bundles are supported.' });
        }

        if (!Array.isArray(entry)) {
            return res.status(400).json({ message: 'Invalid or missing entry array in bundle.' });
        }
        ...
        if (type === 'collection') {
            ...
            return res.status(201).json({ message: 'Collection bundle processed', results });
        }
        ...
        if (type === 'batch') {
            for (const item of entry) {
                ...
                try {
                    ...
                    results.push({ id: result, resourceType: resource.resourceType, status: "success" });
                } catch (error) {
                    results.push({
                        resourceType: resource?.resourceType,
                        status: "error",
            ...
            return res.status(200).json({ message: 'Batch bundle processed', results });
        }
    }
};
